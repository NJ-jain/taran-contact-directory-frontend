...
import { CirclePlus, Crown, FilePenLine, ImagePlus, ImageUp, Pencil, X } from "lucide-react";
import Cropper from "react-easy-crop";
import getCroppedImg from "../utils/cropImage";
import { createMemberThunk, updateMemberThunk } from "../features/member/memberSlice";
...
const Profile = () => {
    ...
    const bannerUrl = useMemo(() => {
        return userData?.banner ? `${userData.banner}` : "/path/to/fallback-banner.jpg";
    }, [userData?.banner]);
                                    ...
                                    <div className="px-6">
                                        ...
                                        <div className="py-10">
                                            ...
                                            <div className="flex flex-wrap gap-4">
                                                {userData.membersArray && userData.membersArray.map((member) => (
                                                    <div key={member._id} className="card bg-base-100 shadow-md hover:shadow-2xl w-72">
                                                        <figure className="px-4 pt-4">
                                                            ...
                                                            />
                                                            {member.familyHead ?
                                                                <div className="tooltip absolute top-5 left-5" data-tip="Family head">
                                                        ...
                                                        <div className="card-body">
                                                            <h2 className="card-title text-sm">
                                                                <span className="capitalize">{member.firstName}</span> <span className="capitalize">{member.lastName}</span>
                                                            </h2>
                                                            <p className="text-xs"><strong>Phone Number:</strong> {member.phoneNumber}</p>
                                                            <p className="text-xs"><strong>Email:</strong> {member.email}</p>
                                                            <p className="text-xs"><strong>Address:</strong> {member.address}</p>
                                                        </div>
                                                    </div>
                                                ))}
                                                <div
                                                    className="card bg-base-100 shadow-md hover:shadow-2xl flex items-center cursor-pointer justify-center w-72"
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-30">
                        <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl transform transition-all duration-300 ease-in-out">
                            <div className="flex items-center justify-between mb-10 w-full">
                                <h3 className="text-2xl font-bold text-center text-gray-800"> {selectedMember._id ? "Edit Member Details" : "Add New Member"}</h3>
                                <X size={20} color="red" onClick={closeMemberModal} className="hover:cursor-pointer" />
                            </div>
                            {
                                selectedMember.dp ?
                                    <img
                                        ...
                                        onClick={() => document.getElementById('memberImageInput').click()} // Trigger file input click
                                    /> :
                                    <ImageUp size={48} className="w-full h-48 object-contain rounded-lg mb-4 cursor-pointer" onClick={() => document.getElementById('memberImageInput').click()} />
                            }
                             {/* Profile Picture Field */}
                             {fieldErrors.dpFile && <p className="text-red-500 text-xs text-center">{fieldErrors.dpFile}</p>}
                            <input
                                type="file"
                                id="memberImageInput"
                                style={{ display: 'none' }}
                                onChange={(e) => {
                                    ...
                                    if (file) {
                                        const imageUrl = URL.createObjectURL(file);
                                        setSelectedMember((prev) => ({ ...prev, dp: imageUrl, dpFile: file })); // Store file object
                                    }
                                }}
                            />
                            <div className="space-y-2">
                                ...
                                <label className="block text-gray-600">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-2 border rounded"
                                        onChange={(e) => {
                                            setSelectedMember((prev) => ({ ...prev, firstName: e.target.value }));
                                ...
                                <label className="block text-gray-600">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-2 border rounded"
                                        onChange={(e) => {
                                            setSelectedMember((prev) => ({ ...prev, lastName: e.target.value }));
                                ...
                                <label className="block text-gray-600">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-2 border rounded"
                                        onChange={(e) => {
                                            setSelectedMember((prev) => ({ ...prev, address: e.target.value }));
                                ...
                                <label className="block text-gray-600">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-2 border rounded"
                                        onChange={(e) => {
                                            setSelectedMember((prev) => ({ ...prev, email: e.target.value }));
                                ...
                                <label className="block text-gray-600">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-2 border rounded"
                                        onChange={(e) => {
                                            setSelectedMember((prev) => ({ ...prev, phoneNumber: e.target.value }));
                                ...
                                <label className="block text-gray-600">
                                    ...
                                    <input
                                        ...
                                        className="w-full p-2 border rounded"
                                        onChange={(e) => {
                                            setSelectedMember((prev) => ({ ...prev, dob: e.target.value }));
